...
const CartItem =({item}) =>{
   ...
   const removeItemFromCart =(id) =>{
    ...
    return (
        <Component>
            <LeftComponent>
                 <img src={item.url} alt= "itemPhoto" style={{width:'110px' ,height:'110px'}}/>
                 < GroupedButton/>
            </LeftComponent>
            <Box style={{margin:'20px'}}>
                 <Typography>{addEllipsis(item.title.longTitle) }</Typography>
                 <SmallText>Seller:RetailNet
                  <Box component ='span' ><img src = {fassured } alt ="flipkartAssured" style={{width:'50px' , marginLeft:'10px'}} /></Box>
                 </SmallText>
